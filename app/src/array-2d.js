(function (root) {
   ...
   * coord data methods to help with data maniuplation and searching (getAdjacent, findNearest, filter,
   * getWithinSquareRadius etc.)
   * @class Array2d
   * @constructor
  ...
  Array2d.prototype = {
     ...
     * @param {bool} [settings.withCoords=false] - If true the returned array will include the coords of each value
     *   ([{x: 0, y: 0, value: 1}, ...])
     * @param {bool} [settings.withDiagonals=true] - If true diagonals will be included.
     * @param {Function} [settings.filter=false] - A function to filter the values returned (function(value, x, y){
     *   return true;})
     ...
     * @param {bool} [settings.withCoords=false] - If true the returned array will include the coords of each value
     *   ([{x: 0, y: 0, value: 1}, ...])
     * @param {bool} [settings.includeTarget=false] - If true the value of the coordinates given will be included in
     *   the returned array.
    ...
    /**
     * Retrieves an array of values of coords along a line starting at point 0 and crossing point 1 until it hits the
     * edge of the 2d array or a coord value returning true when passed to the condtion function.
     ...
     * @param {Function} [condition=false] - A function to determine when to end the line. A coord value returning true
     *   when passed to the function will end the line. (function(value, x, y){ return true;})
     * @param {bool} [withCoords=false] - If true the returned array will include the coords of each value ([{x: 0, y:
     *   0, value: 1}, ...])
     * @return {Array} An array of coord values.
     */
    getLineThrough: (x0, y0, x1, y1, condition, withCoords) => {
      const coords = withCoords || false;
    ...
    /**
     * Retrieves an array of the nearest coord values meeting checked requirements. If multiple coord values were
     * matched at the same nearest distance, the returned array will contain multiple matched coord values. Used for
     * projecting path of ranged attacks, pushed entities, ect.
     * @method getNearest
     ...
     * @param {Object} [params] -
     * @param {Number} [params.maxRadius=1] - Maxium search radius from given coord.
     * @param {Function} [params.filter=false] - A function to determine when the desired coord value is matched. A
     *   coord value returning true when passed to the function would be added to the list of results. (function(value,
     *   x, y){ return true;}) If no check function is provided any tile with a truthy value will be matched.
     * @param {bool} [params.withCoords=false] - If true the returned array will include the coords of each value
     *   ([{x: 0, y: 0, value: 1}, ...])
     * @return {Array} An array of coord values within a square radius of the given coords.
     */
    getNearest: (tileX, tileY, params) => {
      const settings = params || {};
      const maxRadius = settings.maxRadius || 1;
      const filter = settings.filter || false;
      const withCoords = settings.withCoords || false;
      const results = [];

      var checkVal = (val, x, y) => {
      ...
      while (currentDistance <= maxRadius) {
        ...
        const minX = tileX - currentDistance;
        const maxX = tileX + currentDistance;
        const minY = tileY - currentDistance;
        const maxY = tileY + currentDistance;
        const len = currentDistance * 2 + 1;
     ...
     * @method filter
     * @param {Function} filter - A function to determine if a value is to be included in results (returns true).
     *   (function(value, x, y){ return true;})
     * @param {bool} [withCoords=false] - If true the returned array will include the coords of each value ([{x: 0, y:
     *   0, value: 1}, ...])
